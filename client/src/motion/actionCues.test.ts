...
describe("planActionMotion", () => {
  ...
  it("keeps rejected slaps outside the discard and attaches penalties only to wrong slaps", () => {
    ...
    expect(wrong.cues[0]).toMatchObject({ returnToSource: true, returnDelayMs: MOTION_TIMING.rejectedSlapHoldMs, returnDurationMs: MOTION_TIMING.rejectedSlapReturnMs });
    expect(late.cues[0]).toMatchObject({ returnToSource: true, returnDelayMs: MOTION_TIMING.rejectedSlapHoldMs, returnDurationMs: MOTION_TIMING.rejectedSlapReturnMs });

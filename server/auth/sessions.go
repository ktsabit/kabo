...
func (s *Sessions) Create(identity Identity, roomID string, lifetime time.Duration) (string, error) {
	...
	s.mu.Lock()
	s.data[id] = session{Identity: identity, RoomID: roomID, ExpiresAt: time.Now().Add(lifetime)}
	s.mu.Unlock()
	return id, nil
}
...
func (s *Sessions) Resolve(id, roomID string) (Identity, error) {
	...
	if !ok || time.Now().After(value.ExpiresAt) {
		delete(s.data, id)
		return Identity{}, errors.New("session is missing or expired")
	}
	if value.RoomID != roomID {
		return Identity{}, errors.New("session is not valid for this room")
	}

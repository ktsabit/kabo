...
Slap requests are serialized by the room lock and carry the current `discardEventId`; the first valid request advances that ID and wins the single slap slot for that discard. A later stale request, or any request after the top card has itself been slapped, is a `wrong_slap`, adds a penalty card, and publishes a transient `action` with `kind: "wrong_slap"`, the attempted target, and its card so every client can animate the public face-up penalty reveal. The card value is not added to the target player's normal private snapshot.
